...
const VALID_STATUSES = new Set<string>([
  "pending", "placed", "processing", "dispatched", "completed", "cancelled",
]);

const VALID_SORT_FIELDS = new Set<string>([
  "created_at", "total", "status", "customer", "email",
]);
...
const OrdersPage = async ({ searchParams }: OrdersPageProps): Promise<React.ReactElement> => {
  ...
  const status: OrderStatus | "ALL" =
    rawStatus && VALID_STATUSES.has(rawStatus) ? (rawStatus as OrderStatus) : "ALL";
  ...
  try {
    [result, completedCount, processingCount, cancelledCount] = await Promise.all([
      getOrders({ search, status, isPaid, dateFrom, dateTo, sortField, sortDir, page, pageSize }),
    ...
    <main className="min-h-screen bg-background">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <header className="mb-8 flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary/10 border border-border">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 mb-6">
          ...
          <StatCard label="Processing" value={processingCount} accent="processing" />
          <StatCard label="Cancelled" value={cancelledCount} accent="cancelled" />
        </div>
  ...
  return (
    <div className="p-5 space-y-4">
      ...
      <div className="flex gap-2">
        <div className="h-9 flex-1 max-w-sm animate-pulse rounded-md bg-muted" />
